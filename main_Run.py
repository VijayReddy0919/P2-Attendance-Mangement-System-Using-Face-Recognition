    ...
    def fill_attendance():
        ...
        if subb == '':
            ...
            def create_csv():
                ...
                csv_name = r"C:\Users\vr360\Attendace_management_system\Attendance\Manually Attendance" + DB_table_name + '.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(
                    r'explorer /select,"C:\Users\vr360\Attendace_management_system\Attendance\Manually Attendance\---check attendance ---"')
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                root.configure(background='grey80')
                cs = 'C:/Users/vr360/Attendace_management_system/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(
            r'explorer /select,"C:\Users\vr360\Attendace_management_system\Attendance/-------Check atttendance-------"')

    attf = tk.Button(windo, text="Check Sheets", command=Attf, fg="white", bg="#34495E", width=15, height=2,
                 activebackground="white", font=('times', 14, 'bold'))
    attf.place(x=230, y=255)

    sub = tk.Label(windo, text="Enter Subject:", width=15, height=2, fg="white", bg="#2980B9", font=('times', 15, 'bold'))
    sub.place(x=30, y=100)

    tx = tk.Entry(windo, width=25, bg="white", fg="black", font=('times', 16), borderwidth=2, relief="solid")
    tx.place(x=250, y=105)

    fill_a = tk.Button(windo, text="Fill Attendance", fg="white", command=Fillattendances, bg="#1ABC9C", width=20, height=2,
                   activebackground="white", font=('times', 15, 'bold'))
    ...
    def log_in():
        ...
        if username == 'pragya':
            if password == 'pragya123':
                ...
                cs = "C:/Users/vr360/Attendace_management_system/StudentDetails/StudentDetails.csv"
